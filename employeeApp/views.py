...
def employee_meal_choose(request, user_uuid):
    ...
    if request.method == "POST":
        if not request.POST.get("option_id") or  not request.POST.get("employee_id") or  not request.POST.get("menu_id"):
        ...
        if len( Order.objects.filter(employee_identifier=employee, menu__id=choosen_menu.id) ) == 0:
            order = Order(
                menu=choosen_menu,
            ...
            order = Order.objects.filter(employee_identifier=employee, menu=choosen_menu)[0]
